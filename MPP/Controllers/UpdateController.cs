...
namespace MPP.Controllers
{
    ...
    //    [HttpPost]
    //    public ActionResult UpdateSelectedRecords(FormCollection form, string command)
    //    {
    //        string outMsg = Constant.statusSuccess;
    ...
    //            string strInputRowIds = Convert.ToString(Session["INPUTROWIDS"]);
    //            int entityTypeId = Convert.ToInt32(Session["EntityTypeID"]);
    ...
    //            {
    //                attributeList = objMenuViewModel.ShowAttributeData(entityTypeId, "", userName[1].ToUpper(), out outMsg);
    //            }
    ...
    //                string strRejectFileName = "I" + userName[1] + Convert.ToString(Session["EntityName"]) + formatedDate + ".csv";
    ...
    //        return outMsg;
    //    }
    //    private string ValidateData(FormCollection form, List<Entity_Type_Attr_Detail> attributeList)
    //    {
    //        string outMsg = Constant.statusSuccess;
    ...
    //            foreach (var data in attributeList)
    //            {
    //                if (data.DISPLAY_TYPE.ToUpper() == "CHECKBOXLIST" && form[data.ATTR_NAME].Contains(','))
    //                {
    //                    string[] splitValue = form[data.ATTR_NAME].Split(',');
    //                    for (int i = 0; i < splitValue.Length; i++)
    //                    {
    ...
    //                else
    //                {
    //                    if (!string.IsNullOrEmpty(form[data.ATTR_NAME]))
    //                        isNotNulll = true;
    //                }
    //            }
    //            if (!isNotNulll)
